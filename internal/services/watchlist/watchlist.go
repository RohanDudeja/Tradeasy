...
func AddStockEntry(req AddStockRequest, watchlistId int) (res AddStockResponse, err error) {
	...
	if err != nil {
		...
		err = database.GetDB().Table("user_watchlist").Model(userWatchlist).
			Updates(&userWatchlist).Error
		if err != nil {
			return res, errors.New("stock not added")
		}
	}
...
func SortWatchlist(SortReq SortRequest) (SortRes SortResponse, err error) {
	...
	err = database.GetDB().Raw("SELECT * FROM user_watchlist ORDER BY stock_name ASC").Scan(&wl).Error
	if err != nil {
